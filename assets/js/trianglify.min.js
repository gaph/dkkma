...
"undefined" != typeof module && module.exports && (d3 = require("d3"), jsdom = require("jsdom"), document = new(jsdom.level(1, "core").Document), XMLSerializer = require("xmldom").XMLSerializer, btoa = require("btoa"), module.exports = Trianglify), Trianglify.randomColor = function() {
  ...
  return function(c, b) {
    var t = d3.scale.linear().range(f).domain(d3.range(0, d, d / f.length)),
      i = d3.scale.linear().range(e).domain(d3.range(0, a, a / e.length));
    return d3.interpolateRgb(t(c), i(b))(.5)
  }
  ...
  var f = this.options,
    ...
    a = d3.range(e * d).map(function(d) {
      var a = d % e,
        c = Math.floor(d / e),
        b = Math.round(-f.bleed + a * f.cellsize + Math.random() * (f.cellsize - 2 * f.cellpadding) + f.cellpadding),
        t = Math.round(-f.bleed + c * f.cellsize + Math.random() * (f.cellsize - 2 * f.cellpadding) + f.cellpadding);
      return [b, t]
    });
  ...
  if (f.noiseIntensity > .01) {
    ...
    b.append("feTurbulence").attr("type", "fractalNoise").attr("in", "fillPaint").attr("fill", "#F00").attr("baseFrequency", .7).attr("numOctaves", "3").attr("stitchTiles", "stitch");
  ...
  return this.polys.forEach(function(d) {
    var a = (d[0][0] + d[1][0] + d[2][0]) / 3,
      b = (d[0][1] + d[1][1] + d[2][1]) / 3,
      t = e(a, b),
      i = c.append("path").attr("d", "M" + d.join("L") + "Z").attr({
        fill: t,
        stroke: t
      });
    1 != f.fillOpacity && i.attr("fill-opacity", f.fillOpacity), 1 != f.strokeOpacity && i.attr("stroke-opacity", f.strokeOpacity)
  }), a.node()
}, Trianglify.Pattern.prototype.append = function() {
  document.body.appendChild(this.svg)
